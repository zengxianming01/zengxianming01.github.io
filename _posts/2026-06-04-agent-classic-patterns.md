---
layout: post
title: "智能体经典范式构建 — ReAct / Plan-and-Solve / Reflection"
date: 2026-06-04
...
tags:
  ...
  - Reflection
  - Agent
excerpt: "从零实现 ReAct、Plan-and-Solve 与 Reflection 三种经典智能体范式，理解智能体如何组织思考、行动、计划和自我修正。"
source_path: "/Users/zengxianming/Documents/Obsidian Vault/wiki/智能体/智能体经典范式构建.md"
type: concept
---
...
我们可以将这个过程形式化地表达出来，如图1.1所示。具体来说，在每个时间步 $t$ ，智能体的策略（即大语言模型 $\pi$ ）会根据初始问题 $q$ 和之前所有步骤的"行动-观察"历史轨迹 $((a_1,o_1),\dots,(a_{t-1},o_{t-1}))$ ，来生成当前的思考 $th_t$ 和行动 $a_t$ ：
...
图 1.1 ReAct 范式中的"思考-行动-观察"协同循环
...
### 1.3.1 Plan-and-Solve 的工作原理

Plan-and-Solve Prompting 由 Lei Wang 在2023年提出 [2] 。其核心动机是为了解决思维链在处理多步骤、复杂问题时容易"偏离轨道"的问题。

与 ReAct 将思考和行动融合在每一步不同，Plan-and-Solve 将整个流程解耦为两个核心阶段，如图1.2所示：
...
图 1.2 Plan-and-Solve 范式的两阶段工作流
...
如图1.3所示，这个循环可以重复进行多次，直到反思阶段不再发现新的问题，或者达到预设的迭代次数上限。我们可以将这个迭代优化的过程形式化地表达出来。假设 $O_i$ 是第 $i$ 次迭代产生的输出（ $O_0$ 为初始输出），反思模型 $\pi_{\text{reflect}}$ 会生成针对 $O_i$ 的反馈 $F_i$ ：
$$
...
图 1.3 Reflection 机制中的"执行-反思-优化"迭代循环
...
1. **有效的"批判"是优化的前提**:在第一轮反思中，由于我们使用了"极其严格"且专注于"算法效率"的提示词，智能体没有满足于功能正确的初版代码，而是精准地指出了其 `O(n * sqrt(n))` 的时间复杂度瓶颈，并提出了算法层面的改进建议——埃拉托斯特尼筛法。
...
本章探讨的三种范式，代表了智能体解决问题的三种不同策略，如表1.1所示。在实际应用中，选择哪一种，取决于任务的核心需求:

表 1.1 不同 Agent Loop 的选择策略

![不同 Agent Loop 的选择策略](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-4.png)
...
6. 提示词工程是影响智能体最终效果的关键技术。本章展示了多个精心设计的提示词模板。请分析:
   - 对比4.2.3节的 `ReAct` 提示词和4.3.2节的 `Plan-and-Solve` 提示词，它们存在结构设计上的明显不同，这些差异是如何服务于各自范式的核心逻辑的？
